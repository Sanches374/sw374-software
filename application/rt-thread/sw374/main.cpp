...
// 读取配置文件
    ...
    while (get_config_info(&wifissid, &wifipwd, &tcp_port, &sw374version) != 0)
    {
        // 配置文件读取失败
        rt_thread_mdelay(1000);
        ...
        if (read_config_file_count ++ >= READ_CONFIG_FILE_MAX_TIME)
        {
            ...
            // if (p_logpanel) p_logpanel->updateLogDisplay(std::string("Failed to read config file after ") + std::to_string(int(READ_CONFIG_FILE_MAX_TIME)) + std::string(" attempts! SW374_main exit!")); //
            goto cleanup;
        }
    }
    LOG_I("Configuration loaded successfully");
    // if (p_logpanel) p_logpanel->updateLogDisplay("Configuration loaded successfully!");
    LOG_I("Firmware version: %s", sw374version);
    // if (p_logpanel) p_logpanel->updateLogDisplay(std::string("Firmware version:") + std::string(sw374version));
...
// 建立TCPserver
    ...
    if (wifissid) free(wifissid);
    if (wifipwd) free(wifipwd);
    if (sw374version) free(sw374version);
    delete p_logpanel;
    p_panel = new LVGL_SW_F1_Panel_1(); // 创建一个新的LVGL_SW_F1_Panel_1对象
    return 0;
